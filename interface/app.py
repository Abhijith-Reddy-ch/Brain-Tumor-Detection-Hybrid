...
USE_QNN = os.environ.get("USE_QNN", "true").lower() == "true"
...
if BACKEND == "onnx":
    try:
        ...
        MODEL = ("torch",) + load_torch_model(use_qnn=USE_QNN)
        print(f"Loaded PyTorch model (QNN={'enabled' if USE_QNN else 'disabled'}).")
else:
    MODEL = ("torch",) + load_torch_model(use_qnn=USE_QNN)  # ("torch", model, classes, device)
    print(f"Loaded PyTorch model (QNN={'enabled' if USE_QNN else 'disabled'}).")
